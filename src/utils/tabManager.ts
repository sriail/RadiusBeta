...
export class TabManager {
    ...
    async init() {
        ...
        this.setupThemeMessageListener();
    ...
    private renderTab(tab: Tab) {
        ...
        closeBtn.className =
            "ml-1 p-0.5 rounded-full hover:bg-(--muted-foreground) opacity-0 group-hover:opacity-60 hover:!opacity-100 transition-opacity";
    ...
    private renderTabContent(tab: Tab) {
        ...
        if (tab.url === "about:settings") {
            // Render settings page using iframe
            ...
            tab.title = "Settings";
            tab.favicon =
                "https://ssl.gstatic.com/images/branding/product/1x/gsa_settings_96dp.png";
    ...
    private handleIframeLoad(tabId: string) {
        ...
        try {
            ...
            if (iframeDoc) {
                ...
                const faviconLink = iframeDoc.querySelector(
    ...
    private async fetchFavicon(tabId: string, url: string) {
        try {
            const data = await this.bareClient.fetch(
    ...
    private activateTab(tabId: string) {
        this.tabs.forEach((tab) => {
            ...
            if (tabEl) {
                if (tab.isActive) {
                    tabEl.className = tabEl.className.replace(
                        "bg-(--muted) mt-1 hover:bg-(--accent)",
    ...
    private navigateTab(tabId: string, input: string) {
        ...
        if (url.startsWith("about:")) {
            ...
            if (isURL) {
                ...
                const searchEngine =
                    this.storage.getVal("searchEngine") || "https://www.google.com/search?q=";
    ...
    private setupEventListeners() {
        ...
        document
            .getElementById("nav-bookmark")
            ?.addEventListener("click", () => this.addBookmark());
        document
            .getElementById("nav-fullscreen")
    ...
    private setupThemeMessageListener() {
